...
CREATE TABLE generated_posts (
    ...
    source_url TEXT,
    buffer_post_id TEXT,
    error_message TEXT,                  -- why a post landed in 'failed' (Buffer reason etc.)
    model_used TEXT,                     -- which tier wrote it: gemini | openrouter | groq-vision | groq-<model>
    shape_used TEXT,                     -- POST_SHAPES id (v8: archetype id)
    derivation_used TEXT,                -- DERIVATION_MODES id (arithmetic | consequence | omission | ...)
...
-- Quality attribution: "pass-rate / length / outcome by shape (or model) over time".
-- These support the decision to enforce REQUIRE_INFO_DELTA, which needs a
-- pass-rate measured on the model that actually writes, sliced by shape.
CREATE INDEX IF NOT EXISTS idx_generated_posts_shape_date ON generated_posts (shape_used, db_created_at);
CREATE INDEX IF NOT EXISTS idx_generated_posts_model_date ON generated_posts (model_used, db_created_at);
