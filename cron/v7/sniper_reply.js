...
import {
  ...
  stripReasoning, isLikelyEnglish, replyGuardFloor,
} from '../lib/utils.js';
...
function cleanReplyText(raw, sourceText = '') {
  ...
  // The shared must-never-ship floor. Kept in utils.js rather than inlined here so
  // this validator cannot drift behind the feed pipelines again — see the note on
  // replyGuardFloor() for how it drifted the first time.
  const floor = replyGuardFloor(text, sourceText);
  if (floor) {
    console.warn(`  ⚠ Reply rejected: ${floor}`);
    return null;
  }
...
async function generateSniperReply(author, tweetText) {
  const prompt = `You are @M_jawad_yasin — an AI systems engineer and builder on X.
...
2. Adds a sharp engineering nuance, real-world deployment reality, production latency/cost trap, or edge case they omitted.
...
- You have NOT run, tested, benchmarked, or deployed anything here. The only thing you
  have read is the tweet above.
- Do NOT claim hands-on experience. No "I ran this in prod", "in my experience",
  "we tested it", "I've seen teams". These are fabrications and they get rejected.
- Reasoning is welcome and expected. If you point at a trade-off, derive it from the
  facts in the tweet and say why — that reads as expertise, and it is checkable.
- Do NOT cite an authority you were not given. No "reports indicate", "benchmarks show",
  "studies find" — unless the tweet itself is that source.
- Do NOT invent numbers. If the tweet gives a figure you can compute from, compute it
  and show the inputs. Otherwise stay qualitative.

SELF-CHECK BEFORE YOU ANSWER: does your reply contain a claim the tweet does not, and
could you defend that claim without pretending to have run anything? If not, rewrite it.
...
async function main() {
  if (process.env.V7_ENABLED !== 'true') {
    console.log('⏸ v7 is paused. Set V7_ENABLED=true for an explicit manual run.');
    return;
  }
