...
import { validateEnv, replyGuardFloor } from './lib/utils.js';
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      const { data: claimed, error: claimErr } = await supabase
        ...
        .select('id, status, generated_text, original_text, infographic, retry_count')
        .single();
...
/**
 * Mark a post permanently failed after a GUARD rejection — deliberately NOT
 * handleFailure().
 *
 * handleFailure() resets the row to 'approved' with a backoff when retries remain.
 * That is correct for transient faults (a Buffer 5xx) and wrong for a guard
 * rejection, which is deterministic: the same text fails identically on every
 * attempt, so retrying only burns the retry budget and re-enters the queue
 * MAX_RETRIES times for no chance of success.
 */
async function markGuardRejected(post, reason) {
  ...
  const { error } = await supabase
    ...
    .update({
      status: 'failed',
      claimed_at: null,
      infographic: { ...existingInfographic, last_error: `guard: ${reason}` },
    })
    .eq('id', post.id);
  if (error) console.error(`  ✗ Failed to mark guard rejection: ${error.message}`);
  else console.log(`  ✓ Post [${post.id}] marked 'failed' (guard, no retry)`);
}
...
async function runAutoPoster() {
  ...
  try {
    ...
    // 2. Last-line-of-defence guard.
    //
    // This is the queue consumer: the single choke point where a queued row
    // actually goes live on X. It previously ran NO guard at all, so anything that
    // reached the queue published as-is — including rows written by older code
    // before the producer-side finalizer existed.
    //
    // Deliberately the "must never ship" floor only, NOT the full finalizer. A scan
    // of the live queue found 43% of pending rows would fail the full chain, but
    // most of those failures were LENGTH policy — and among them were perfectly
    // good posts ("Hot take: Opus 5 isn't king", 77 chars; "Harvard & UIUC talent
    // discover a 3rd pretraining axis: 6.2x sample efficiency", 110 chars). Length
    // is the producer's policy to enforce, not the consumer's. The consumer owns
    // the absolute floor: prompt leaks, placeholders, fabricated evidence.
    const floor = replyGuardFloor(post.generated_text || '', post.original_text || '');
    if (floor) {
      console.error(`  ⛔ Refusing to publish [${post.id}] — ${floor}`);
      await markGuardRejected(post, floor);
      return;
    }

    // 3. Detect thread vs single post
