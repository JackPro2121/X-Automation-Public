...
function getArchetypeByHour() {
  ...
  if (currentHour >= 16 && currentHour <= 19) {
    // ~1:30 PM EDT (US Lunch break / builder debate)
    return Math.random() > 0.5 ? 'dev_identity_crisis' : 'community_roast_magnet';
  } else if (currentHour >= 23 || currentHour <= 2) {
    // ~8:30 PM EDT (US Evening tech philosophy & viral debate)
    return Math.random() > 0.5 ? 'vibe_coding_irony' : 'generational_shift';
  }
