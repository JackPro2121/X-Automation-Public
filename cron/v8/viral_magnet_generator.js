...
export async function generateViralMagnet(openRouterKeys, groqKeys, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    // 1. Gemini (primary). Generous budget — the primary model returns empty
    ...
    if (isGeminiConfigured()) {
      try {
        ...
        if (raw) {
          ...
          console.log(`  ✓ Gemini [${getLastGeminiModel()}] responded`);
        }
      } catch (err) {
    ...
    // 3. Groq (tertiary) — model IDs are config-driven via GROQ_TEXT_MODELS so a
