...
import { smartTrimToTarget, THREAD_TWEET_TARGET_CHARS } from '../tweetLimits.js';
import { isMetaTextCaption, isPlaceholderText } from './utils.js';

// ─── Publish-time length safety ───────────────────────────────────────────────
//
// This used to be `smartTrimToTarget(text)` with no target, which fell back to
// `TWEET_TARGET_CHARS` (450). Every post was therefore silently re-cut to 450
// chars at publish time — AFTER the quality guards had validated the full text —
// so ~12% of live posts lost their ending and their CTA while the DB stored the
// intact version. Nothing re-validated the shortened string.
//
// The pipeline is now responsible for trimming (see finalizePostText in utils.js).
// This layer is a LAST-RESORT safety net only: it trims to X_SAFE_MAX_CHARS, never
// below, and it says so loudly so silent data loss can never happen again.
/**
 * @param {string} text
 * @param {string} [label='post']
 * @param {number} [cap=X_SAFE_MAX_CHARS]
 * @returns {string}
 */
function enforcePublishLimit(text, label = 'post', cap = X_SAFE_MAX_CHARS) {
  if (!text) return text;
  if (text.length <= cap) return text;

  const trimmed = smartTrimToTarget(text, cap);
  console.warn(
    `  ⚠ PUBLISH TRIM: ${label} was ${text.length} chars, cut to ${trimmed.length} ` +
    `(safe cap ${cap}). The pipeline should have trimmed this BEFORE save — ` +
    `check finalizePostText() in cron/lib/utils.js.`
  );
  return trimmed;
}

/**
 * Apply the platform hard limit as the final, absolute backstop.
 * @param {string} text
 * @returns {string}
 */
function enforcePlatformLimit(text) {
  if (!text || text.length <= X_MAX_CHARS) return text;
  console.warn(`  ⚠ PUBLISH TRIM: text exceeded the platform limit (${text.length} > ${X_MAX_CHARS}) — hard cutting.`);
  return text.substring(0, X_MAX_CHARS - 1).trim() + '…';
}
...
/**
 * Public pre-validation: check whether a media URL is within Buffer's limits
 * (Images <= 4.5MB, GIFs <= 15MB).
 * Used during candidate selection so the pipeline skips oversized media
 * and selects the next high-quality candidate instead of failing at Buffer.
 * @param {string|null} mediaUrl
 * @returns {Promise<boolean>}
 */
export async function isMediaWithinLimits(mediaUrl) {
  if (!mediaUrl) return true;
  if (isLikelyLargeGif(mediaUrl)) return false;
  return isImageWithinLimit(mediaUrl);
}
...
async function executeBufferRequest(mutation, variables, retryCount = 0) {
  ...
  try {
    ...
    if (result?.message) {
      if (result.message.includes('cannot exceed 280 characters') && variables?.input?.text?.length > 280 && retryCount < 1) {
        console.warn(`  ⚠ Buffer channel not yet synced for Premium (>280 chars) — auto-trimming to 240 chars and retrying...`);
        const trimmed = smartTrimToTarget(variables.input.text, 240);
        const safeTrimmed = trimmed.length > 280 ? trimmed.substring(0, 277).trim() + '…' : trimmed;
        const retryVars = {
          ...variables,
          input: {
            ...variables.input,
        ...
        return executeBufferRequest(mutation, retryVars, retryCount + 1);
      }
...
export async function postSingleToBuffer(text, imageUrl = null) {
  ...
  // F1c (final defense): reject leaked meta-text captions before they go live
  if (isMetaTextCaption(text)) {
    console.warn('  ⚠ Blocking Buffer post — caption is meta-text (leak)');
    return { success: false, reason: 'Caption is meta-text (leak)' };
  }

  // F1d (final defense): reject raw template placeholders like "[Punchy closing…]"
  if (isPlaceholderText(text)) {
    console.warn('  ⚠ Blocking Buffer post — caption contains an unfilled template placeholder');
    return { success: false, reason: 'Caption contains a template placeholder' };
  }
  ...
  const safeText = enforcePlatformLimit(enforcePublishLimit(text, 'single tweet'));
...
export async function postThreadToBuffer(tweets, imageUrl = null) {
  ...
  // F1c/F1d (final defense): reject leaked meta-text or unfilled placeholders.
  // Previously the thread path had no leak guard at all, unlike the single path.
  for (const t of validTweets) {
    if (isMetaTextCaption(t)) {
      console.warn('  ⚠ Blocking Buffer thread — a tweet is meta-text (leak)');
      return { success: false, reason: 'Thread tweet is meta-text (leak)' };
    }
    if (isPlaceholderText(t)) {
      console.warn('  ⚠ Blocking Buffer thread — a tweet contains an unfilled template placeholder');
      return { success: false, reason: 'Thread tweet contains a template placeholder' };
    }
  }
  ...
  // Enforce per-tweet target + platform cap on every tweet in the thread
  const safeTweets = validTweets.map((t) =>
    enforcePlatformLimit(enforcePublishLimit(t, 'thread tweet', THREAD_TWEET_TARGET_CHARS))
  );
...
export async function postVideoToBuffer(text, videoUrl, imageUrl = null) {
  ...
  // F1c (final defense): reject leaked meta-text captions before they go live
  if (isMetaTextCaption(text)) {
    console.warn('  ⚠ Blocking Buffer video post — caption is meta-text (leak)');
    return { success: false, reason: 'Caption is meta-text (leak)' };
  }
  if (isPlaceholderText(text)) {
    console.warn('  ⚠ Blocking Buffer video post — caption contains an unfilled template placeholder');
    return { success: false, reason: 'Caption contains a template placeholder' };
  }
  ...
  const safeText = enforcePlatformLimit(enforcePublishLimit(text, 'video tweet'));
...
export async function postToQueue(text, imageUrl = null) {
  ...
  // F1c: Reject leaked meta-text captions (shared detector — see utils.js)
  if (isMetaTextCaption(text)) {
    return { success: false, reason: 'Caption is meta-text (leak)' };
  }
  if (isPlaceholderText(text)) {
  ...
  const safeText = enforcePlatformLimit(enforcePublishLimit(text, 'queued tweet'));
...
export async function postQuoteTweet(comment, tweetId) {
  ...
  if (isMetaTextCaption(comment)) {
    return { success: false, reason: 'Comment is meta-text (leak)' };
  }
  if (isPlaceholderText(comment)) {
    return { success: false, reason: 'Comment contains a template placeholder' };
  }

  const safeComment = enforcePlatformLimit(enforcePublishLimit(comment, 'quote tweet comment'));
