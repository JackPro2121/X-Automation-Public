...
export async function fetchXPosts(apifyKeys, accounts = AI_ACCOUNTS, maxPosts = 6, maxAccounts = 3) {
  ...
  for (const account of selectedAccounts) {
    ...
    try {
      ...
      if (posts.length > 0) successfulAccounts++;
      allPosts.push(...posts);
  ...
  console.log(`  ✓ ${mediaPosts.length} qualifying posts from ${allPosts.length} total (${successfulAccounts}/${attemptedAccounts} accounts yielded posts)`);

  if (attemptedAccounts > 0 && successfulAccounts === 0) {
    console.warn(`  ⚠ Warning: 0 of ${attemptedAccounts} accounts returned posts. Apify actor run or keys may be failing.`);
  }
  ...
  result.scannedAccountsCount = attemptedAccounts;
  return result;
}
